...
const GenerationContext = struct {
    ...
    pub fn writeToStdOut(self: *Self) !void {
        ...
        while (it.next()) |entry| {
            ...
            ret.name = try std.fmt.allocPrint(allocator, "{?s}.pb.zig", .{try self.packageNameToOutputFileName(entry.key_ptr.*)});
